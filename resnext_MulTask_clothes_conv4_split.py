...
class ResNext(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, seg=False, elastic=False, se=False):
        ...
        self.layer3_1 = self._make_layer_3_1(block, 256, layers[2], stride=2, elastic=elastic, se=se) # 加入elastic结构
        self.layer3_2 = self._make_layer_3_2(block, 256, layers[2], stride=2, elastic=elastic, se=se) # 加入elastic结构
        """分割部分
        if seg:
        ...
        """
        # 非分割部分
        self.layer4_1 = self._make_layer_4_1(block, 512, layers[3], stride=2, elastic=False, se=se) # 最后一部分没有使用elastic结构
        self.layer4_2 = self._make_layer_4_2(block, 512, layers[3], stride=2, elastic=False, se=se) # 最后一部分没有使用elastic结构
...
#            print("planes * block.expansion: ", planes * block.expansion)
        ...
        self.inplanes = planes * block.expansion # 有变化
...
#########################################################################################
###layer_3
    def _make_layer_3_1(self, block, planes, blocks, stride=1, elastic=False, se=False):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
#            print("self.inplanes: ", self.inplanes)
#            print("planes * block.expansion: ", planes * block.expansion)
        ...
        # 该部分是将每个blocks的第一个residual结构保存在layers列表中
        layers.append(block(self.inplanes, planes, self.cardinality, stride, downsample=downsample, norm=self._norm, elastic=elastic, se=se))
        self.inplanes_3_1 = planes * block.expansion # 有变化
        # 该部分是将每个blocks的剩下residual 结构保存在layers列表中，这样就完成了一个blocks的构造
        for i in range(1, blocks):
            layers.append(block(self.inplanes_3_1, planes, self.cardinality, norm=self._norm, elastic=elastic, se=se))
        return nn.Sequential(*layers)

    def _make_layer_3_2(self, block, planes, blocks, stride=1, elastic=False, se=False):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
#            print("self.inplanes: ", self.inplanes)
#            print("planes * block.expansion: ", planes * block.expansion)
        ...
        # 该部分是将每个blocks的第一个residual结构保存在layers列表中
        layers.append(block(self.inplanes, planes, self.cardinality, stride, downsample=downsample, norm=self._norm, elastic=elastic, se=se))
        self.inplanes_3_2 = planes * block.expansion # 有变化
        # 该部分是将每个blocks的剩下residual 结构保存在layers列表中，这样就完成了一个blocks的构造
        for i in range(1, blocks):
            layers.append(block(self.inplanes_3_2, planes, self.cardinality, norm=self._norm, elastic=elastic, se=se))
        return nn.Sequential(*layers)

#########################################################################################
###layer_4

    def _make_layer_4_1(self, block, planes, blocks, stride=1, elastic=False, se=False):
        downsample = None
        if stride != 1 or self.inplanes_3_1 != planes * block.expansion:
#            print("self.inplanes: ", self.inplanes)
#            print("planes * block.expansion: ", planes * block.expansion)
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes_3_1, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
                self._norm(planes * block.expansion),
            )
        ...
        layers = list()
        # 该部分是将每个blocks的第一个residual结构保存在layers列表中
        layers.append(block(self.inplanes_3_1, planes, self.cardinality, stride, downsample=downsample, norm=self._norm, elastic=elastic, se=se))
        self.inplanes1 = planes * block.expansion # 有变化
        # 该部分是将每个blocks的剩下residual 结构保存在layers列表中，这样就完成了一个blocks的构造
        for i in range(1, blocks):
            layers.append(block(self.inplanes1, planes, self.cardinality, norm=self._norm, elastic=elastic, se=se))
        return nn.Sequential(*layers)

    def _make_layer_4_2(self, block, planes, blocks, stride=1, elastic=False, se=False):
        downsample = None
        if stride != 1 or self.inplanes_3_2 != planes * block.expansion:
#            print("self.inplanes: ", self.inplanes)
#            print("planes * block.expansion: ", planes * block.expansion)
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes_3_2, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
                self._norm(planes * block.expansion),
            )
        ...
        layers = list()
        # 该部分是将每个blocks的第一个residual结构保存在layers列表中
        layers.append(block(self.inplanes_3_2, planes, self.cardinality, stride, downsample=downsample, norm=self._norm, elastic=elastic, se=se))
        self.inplanes2 = planes * block.expansion # 有变化
        # 该部分是将每个blocks的剩下residual 结构保存在layers列表中，这样就完成了一个blocks的构造
        for i in range(1, blocks):
            layers.append(block(self.inplanes2, planes, self.cardinality, norm=self._norm, elastic=elastic, se=se))
        return nn.Sequential(*layers)
    ...
    def forward(self, x):
        ...
        x1 = self.layer3_1(x) # 有elastic结构
        x2 = self.layer3_1(x) # 有elastic结构
