...
#        pred_mAP = output.data.gt(0.0).long()
        ...
        # top-3指标 计算
        pc_top3, rc_top3, f1c_top3, po_top3, ro_top3, f1o_top3, mAP1, mAP2 = calculate_metrics(y_true, y_pred) 
...
#        f_o = 2 * p_o * r_o / (p_o + r_o)
        ...
        if i % args.print_freq == 0: # 频率刷新
            ...
            print('P_C {:.2f} R_C {:.2f} F_C {:.2f} \t P_O {:.2f} R_O {:.2f} F_O {:.2f} \t mAP1 {:.2f} \t mAP2 {:.2f}'
                  .format(pc_top3, rc_top3, f1c_top3, po_top3, ro_top3, f1o_top3, mAP1, mAP2))
            print()
...
#    tp_size, fn_size = 0, 0
    ...
    no_examples = len(val_loader)*4 # 验证集所有样本数
#    print("no_examples: ", no_examples )  #40504   # 包含batch_size的大小
    no_batches = len(val_loader)  # batch 大小
#    print("no_batches: ", no_batches)  # 10126

    preds = np.empty((no_examples, 80), dtype=np.float32) # 构建空矩阵
    labels = np.empty((no_examples, 80), dtype=np.float32) # 构建空矩阵

    ind_batch=0
    for i, (input, target) in enumerate(val_loader): # 每次一个batch_size的大小
        target = target.cuda(non_blocking=True)
...
#        pred_mAP = output.data.gt(0.0).long()
        ...
        if ind_batch == no_batches - 1:
            preds[no_examples - args.batch_size:no_examples] = y_pred  # 模型预测值
            labels[no_examples - args.batch_size:no_examples] = y_true  # 真实标签
        else:
            preds[ind_batch * args.batch_size:(ind_batch + 1) * args.batch_size] = y_pred
            labels[ind_batch * args.batch_size:(ind_batch + 1) * args.batch_size] = y_true

        ind_batch=ind_batch+1 # 加1
#        no_examples = target.shape[0] # 获得样本数 ——> batch_size的大小
#        output=output.cpu().detach().numpy()
#        print("使用top-3的评价标准")
#        top3 = np.zeros_like(output)
...
#            print()
    ...
    print('--------------------------------------------------------------------')
    # top-3指标 计算
    pc_top3, rc_top3, f1c_top3, po_top3, ro_top3, f1o_top3, mAP1, mAP2 = calculate_metrics(labels, preds) 
    print("验证集的最终结果为：")
    print(' * P_C {:.2f} R_C {:.2f} F_C {:.2f} \t P_O {:.2f} R_O {:.2f} F_O {:.2f} \t mAP1 {:.2f} \t mAP2 {:.2f}'
          .format(pc_top3, rc_top3, f1c_top3, po_top3, ro_top3, f1o_top3, mAP1, mAP2))
    print()
