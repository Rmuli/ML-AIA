...
##############################################################################################
###多任务 构成的是一个 1x4x3 的 Attention Module
#        filter = [64, 256, 512, 1024, 2048]
        ...
        for j in range(1): # 0-1 即2个任务
            if j < 0: # 上面已经有一个，即 0
                self.encoder_att.append(nn.ModuleList([self.att_layer([filter[0], filter[0], filter[0]])]))
                self.linear.append(nn.Sequential(nn.Linear(filter[3], num_classes[j + 1]), nn.Softmax(dim=1))) # 之后的全连接层
            for i in range(3): # 3 # 0-1-2--每个任务有4个 Attention Module,即后面接3个注意力模块
...
##############################################################################################
    ...
    def forward(self, x):
        ...
        """
        # 共享特征
        g_encoder = [0] * 4  # [0, 0, 0, 0]
        # 注意力编码器
        atten_encoder = [0] * 1 # [0, 0]
        for i in range(1):
            atten_encoder[i] = [0] * 4
        for i in range(1):
            for j in range(4):
...
#        print("x:", np.shape(x)) # x: torch.Size([1, 64, 56, 56])
        ...
        g_encoder[0] = self.layer1(x) # 有elastic结构
#        print("g_encoder[1]:", np.shape(g_encoder[1])) # [1, 256, 56, 56]
        g_encoder[1] = self.layer2(g_encoder[0]) # 有elastic结构
#        print("g_encoder[2]:", np.shape(g_encoder[2])) # [1, 512, 28, 28]
        g_encoder[2] = self.layer3(g_encoder[1]) # 有elastic结构
#        print("g_encoder[3]:", np.shape(g_encoder[3])) # [1, 1024, 14, 14]
        g_encoder[3] = self.layer4(g_encoder[2]) # 没有elastic结构
#        print("g_encoder[4]:", np.shape(g_encoder[4])) # [1, 2048, 7, 7]
        ...
        pred2 = self.avgpool(atten_encoder[0][-1][-1]) # atten_encoder[k][-1][-1]
        pred2 = pred2.view(pred2.size(0), -1) # 为了将前面多维度的tensor展平成一维
        out = self.fc(pred2) # 全连接层
