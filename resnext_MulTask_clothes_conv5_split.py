...
class ResNext(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, seg=False, elastic=False, se=False):
        ...
        """
        # 非分割部分
        self.layer4_1 = self._make_layer_1(block, 512, layers[3], stride=2, elastic=False, se=se) # 最后一部分没有使用elastic结构
        self.layer4_2 = self._make_layer_2(block, 512, layers[3], stride=2, elastic=False, se=se) # 最后一部分没有使用elastic结构
...
#            print("planes * block.expansion: ", planes * block.expansion)
    ...
    def _make_layer_1(self, block, planes, blocks, stride=1, elastic=False, se=False):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
#            print("self.inplanes: ", self.inplanes)
#            print("planes * block.expansion: ", planes * block.expansion)
    ...
    def _make_layer_2(self, block, planes, blocks, stride=1, elastic=False, se=False):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
#            print("self.inplanes: ", self.inplanes)
#            print("planes * block.expansion: ", planes * block.expansion)
    ...
    def forward(self, x):
        ...
        x1 = self.layer4_1(x) # 没有elastic结构
        x2 = self.layer4_2(x) # 没有elastic结构
