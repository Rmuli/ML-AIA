...
#    from resnext_MulTask_12 import resnext50_elastic
    ...
    # 不使用elastic结构
    from resnext_MulTask_clothes_conv4_split import resnext50
    model = resnext50(num_classes=data_class)  # 注意类别: 两个任务，对应两个不同的类别数
    
    # 使用elastic结构
#    from resnext_MulTask_clothes_conv4_split import resnext50_elastic 
...
### 继续调用程序
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        }, False, filename='5-clothes-result/' +args.arch + '_checkpoint_'+str(epoch+1)+'.pth.tar') 
...
#        target2 = target2.max(dim=1)[0]   # target: torch.Size([16, 80])
        # compute output 
        output1, output2 = model(input) # 计算模型输出
...
#        target2 = target2.max(dim=1)[0]
        ...
        with torch.no_grad():
            output1, output2 = model(input) # 前向传播
